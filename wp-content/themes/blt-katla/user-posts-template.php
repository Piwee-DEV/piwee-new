...
<div id="site-content" class="clearfix">
	...
	<div id="site-content-column" class="users-posts-section"><?php
		...
		if(isset($_GET['complete'])){ ?>
			...
			elseif(!blt_has_access(blt_get_option('user_posts_access', 'subscriber'))){ ?>
				...
				<div class="users-posts-wrap"><?php
					...
					<form id="blt_posts_form" action="<?php echo blt_get_option('user_posts_page_id') ?>" method="POST" enctype="multipart/form-data">
						...
						?>
						<div class="form-group">
							<label for="blt_post_category"><?php _e('Category', 'bluthemes'); ?></label>
							<select class="form-control" name="blt_post_category" id="blt_post_category"><?php

								foreach($categories as $cat){
									echo '<option value="'.$cat->term_id.'"'.(isset($_POST['blt_post_category']) and $_POST['blt_post_category'] == $cat->term_id ? ' selected="selected"' : '' ).'>'.$cat->name.'</option>';
								}
							?>
							</select>
						</div>
								...
								<div role="tabpanel" id="image-post" class="tab-pane active form-group new-post-list blt_post_files">
									<ul class="list-group">
				  						...
				  						<li class="list-group-item">
											<a href="#new-image" class="blt-add-new-image"><i class="fa fa-plus"></i> <?php _e('Add another image', 'bluthemes') ?></a>
				  						</li>
				  					</ul>	
								</div>
