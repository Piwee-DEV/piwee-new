		...
		switch($ad_spot){
			...
			case 'adsense':
				echo '<div class="'.$ad_spot_name.' '.$ad_spot_name.'-gad">';
					echo do_shortcode('[adsense][/adsense]');
					// echo apply_filters('the_content', do_shortcode('[adsense][/adsense]'));
				echo '</div>';
				break;
		}
	}
								...
								if(isset($image['sizes'])){ ?>
									...
									<li><?php
										...
										if(!empty($source_name[0])){ ?>
											...
											<strong><?php _e('Source:', 'bluthemes'); ?></strong>&nbsp;<a style="display: inline-block; margin-top: 5px;" href="<?php echo !empty($source_url[0]) ? $source_url[0] : '#'; ?>" target="_blank"><?php echo $source_name[0]; ?></a><?php
