            ...
            <script>
                function get_facebook_sdk() {
                    (function (d, s, id) {
                        ...
                        js.id = id;
                        js.src = "//connect.facebook.net/en_GB/all.js#xfbml=1&appId=<?php echo blt_get_option('facebook_app_id'); ?>";
...
function blt_has_access($min_role)
{
    ...
    $min_role = !isset($roles[$min_role]) ? 1 : $roles[$min_role];

    $user = new WP_User(get_current_user_id());

    if (!empty($user->roles) and is_array($user->roles)) {
        foreach ($user->roles as $u_role) {
            $user_role = isset($roles[$u_role]) ? $roles[$u_role] : 0;
        }
    } else {
        $user_role = 0;
    }
