...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( ! empty( $home ) ) {
		echo $before . '<a class="home" href="' . apply_filters( 'woocommerce_breadcrumb_home_url', home_url() ) . '">' . __('Home', 'bd') . '</a>' . $after . $delimiter;
	}
	...
	if ( get_query_var( 'paged' ) )
		echo $before . ' (' . __( 'Page', 'woocommerce' ) . ' ' . get_query_var( 'paged' ) . ')' . $after;
