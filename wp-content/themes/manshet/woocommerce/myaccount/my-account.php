	...
	<table class="my_account_orders">
		...
		<thead>
			<tr>
				...
				<th class="order-actions"><span class="nobr"><?php _e( 'Details', 'woocommerce' ); ?></span></th>
			</tr>
		</thead>
			...
			foreach ( $customer_orders as $customer_order ) {
				...
				?><tr class="order">
					<td class="order-number">
						...
						<time style="display:none;" datetime="<?php echo date('Y-m-d', strtotime( $order->order_date ) ); ?>" title="<?php echo esc_attr( strtotime( $order->order_date ) ); ?>"><?php echo date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) ); ?></time>
					</td>
					<td class="order-date">
