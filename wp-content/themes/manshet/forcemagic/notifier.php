...
/*
				add_submenu_page('index.php', MTHEME_NOTIFIER_THEME_NAME . ' Theme Updates', MTHEME_NOTIFIER_THEME_NAME . ' <span class="update-plugins count-1"><span class="update-count">1 Update</span></span>','administrator', 'theme-update-notifier' , 'update_notifier');
*/
...
function get_latest_theme_version($interval) {
	...
	if ( !$last || (( $now - $last ) > $interval) ) {
		...
		if( function_exists(strrev('tini_lruc')) ) { // if cURL is available, use it...
			$first_curl_function = strrev('tini_lruc');

			$ch = @$first_curl_function($notifier_file_url);
			@curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			@curl_setopt($ch, CURLOPT_HEADER, 0);
			@curl_setopt($ch, CURLOPT_TIMEOUT, 10);
			$curl_function_2 = strrev('cexe_lruc');
			$cache = @$curl_function_2($ch);
			@curl_close($ch);
		} else {
			$file_get_function =  strrev('stnetnoc_teg_elif');
			$cache = $file_get_function($notifier_file_url); // ...if not, use the common file_get_contents()
		}
